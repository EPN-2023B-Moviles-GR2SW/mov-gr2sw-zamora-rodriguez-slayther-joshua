...
class ListaDirectores : ComponentActivity() {
    ...
    private val directorDAO = DirectorDAO(this)
    val data = directorDAO.obtenerDirectores()
        ...
        return when (item.itemId) {
            ...
            R.id.itemEliminarDirector -> {
                ...
                builder.setPositiveButton("Sí") { dialog, which ->
                    val directorSeleccionado = data[positionDirectorSelected]
                    directorDAO.eliminarDirector(directorSeleccionado.id)
                    adapter.notifyDataSetChanged()
                    Toast.makeText(
                        applicationContext,
                        "Director eliminado",
